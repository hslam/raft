...
func (p *proxy) HandleAddPeer(req *AddPeerRequest,res *AddPeerResponse)error {
	if p.node.IsLeader(){
		...
		if leader!=""{
			return p.node.rpcs.Call(p.node.rpcs.AddPeerServiceName(),req,res,leader)
		}
		peers:=p.node.Peers()
		for i:=0;i<len(peers) ;i++  {
			_,leaderId,ok:=p.QueryLeader(peers[i])
			if leaderId!=""&&ok{
				return p.node.rpcs.Call(p.node.rpcs.AddPeerServiceName(),req,res,leaderId)
			}
		}
		return ErrNotLeader
	}
...
func (p *proxy) HandleRemovePeer(req *RemovePeerRequest,res *RemovePeerResponse)error {
	if p.node.IsLeader(){
		...
		leader:=p.node.Leader()
		if leader!=""{
			return p.node.rpcs.Call(p.node.rpcs.RemovePeerServiceName(),req,res,leader)
		}
		peers:=p.node.Peers()
		for i:=0;i<len(peers) ;i++  {
			_,leaderId,ok:=p.QueryLeader(peers[i])
			if leaderId!=""&&ok{
				return p.node.rpcs.Call(p.node.rpcs.RemovePeerServiceName(),req,res,leaderId)
			}
		}
		return ErrNotLeader
	}
